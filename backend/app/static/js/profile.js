...
function setupProfileUpdate() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    // Require OTP for any profile changes:
    // - If email is changed: OTP must be verified for the NEW email.
    // - If only other fields are changed: OTP must be verified for the CURRENT (stored) email.
    if (emailChanged) {
      if (
    ...
    try {
      // Sync email change to DB (requires OTP on server too).
      ...
      // Upsert profile + docs (docs optional here).
...
function setupDocumentUpload() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    // Require OTP verification for documents upload
    if (
    ...
    try {
      ...
      // Save documents to DB as well (so they persist beyond browser storage).
      try {
